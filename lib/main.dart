...
class HomeScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final repo = SupabaseRepository();
    return Scaffold(backgroundColor: Colors.transparent, appBar: AppBar(backgroundColor: Colors.transparent, title: const Text('الغرف'), leading: Builder(builder: (context) => IconButton(icon: const Icon(Icons.menu), onPressed: () => Scaffold.of(context).openDrawer()))), body: StreamBuilder<List<RoomModel>>(stream: repo.getRoomsStream(), builder: (context, snapshot) {
      ...
      return ListView.builder(padding: const EdgeInsets.fromLTRB(16, 8, 16, 100), itemCount: rooms.length, itemBuilder: (context, i) {
        final room = rooms[i];
        final isOfficial = room.roomType == 'official';
        return Padding(padding: const EdgeInsets.only(bottom: 12), child: isOfficial? _buildOfficialRoomCard(context, room) : _buildUserRoomCard(context, room));
      });
    }));
  }
  Widget _buildOfficialRoomCard(BuildContext context, RoomModel room) {
    return GestureDetector(onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => ChatScreen(room: room))), child: Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(24), gradient: const LinearGradient(colors: [Color(0xFF2C1810), Color(0xFF4A3728), Color(0xFF6B4F3A)], begin: Alignment.topLeft, end: Alignment.bottomRight), boxShadow: [BoxShadow(color: AppColors.officialGold.withOpacity(0.4), blurRadius: 20, offset: const Offset(0, 8))], border: Border.all(color: AppColors.officialGold.withOpacity(0.6), width: 2)), child: Stack(children: [Positioned(right: -30, top: -30, child: Icon(Icons.account_balance, size: 120, color: Colors.white.withOpacity(0.03))), Padding(padding: const EdgeInsets.all(20), child: Row(children: [Container(padding: const EdgeInsets.all(14), decoration: BoxDecoration(color: AppColors.officialGold.withOpacity(0.15), borderRadius: BorderRadius.circular(16), border: Border.all(color: AppColors.officialGold, width: 1.5)), child: const Icon(Icons.verified, size: 32, color: AppColors.officialGold)), const SizedBox(width: 16), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Row(children: [Text(room.roomName, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: Colors.white)), const SizedBox(width: 8), Container(padding: const EdgeInsets.symmetric(horizontal: 10, vertical: 3), decoration: BoxDecoration(color: AppColors.officialGold, borderRadius: BorderRadius.circular(8)), child: const Text('رسمية', style: TextStyle(fontSize: 11, fontWeight: FontWeight.bold, color: AppColors.officialWood)))]), const SizedBox(height: 6), Text(room.description?? 'الغرفة الرسمية - أهلاً بالجميع', style: TextStyle(color: Colors.white.withOpacity(0.7), fontSize: 13))])), const Icon(Icons.arrow_forward_ios, size: 16, color: AppColors.officialGold)]))])));
  }
...
class _ChatScreenState extends State<ChatScreen> {
  ...
  Future<void> _sendMessage() async {
    ...
    msgController.clear();
    if (!isOwner && widget.room.roomType!= 'official') await supabase.rpc('increment_points', params: {'room_id': widget.room.id, 'user_id': currentUserId});
  }
  ...
  void _showMembers() {
    showModalBottomSheet(context: context, backgroundColor: Colors.transparent, builder: (context) => Container(decoration: const BoxDecoration(color: AppColors.backgroundEnd, borderRadius: BorderRadius.vertical(top: Radius.circular(24))), padding: const EdgeInsets.all(16), child: Column(children: [Container(width: 40, height: 4, decoration: BoxDecoration(color: AppColors.icon, borderRadius: BorderRadius.circular(2))), const SizedBox(height: 16), const Text('أعضاء الغرفة', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), const SizedBox(height: 16), Expanded(child: FutureBuilder<List<RoomMemberModel>>(future: repo.getRoomMembers(widget.room.id), builder: (context, snapshot) {
      if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
      final members = snapshot.data!;
      return ListView.builder(itemCount: members.length, itemBuilder: (context, i) {
        final member = members[i];
        final rank = i < 3? ['🥇', '🥈', '🥉'][i] : '${i + 1}';
        return ListTile(leading: Stack(children: [CircleAvatar(backgroundImage: member.user?.avatarUrl!= null? NetworkImage(member.user!.avatarUrl!) : null, child: member.user?.avatarUrl == null? Text(member.user?.name[0]?? 'U') : null), if (i < 3) Positioned(right: -2, bottom: -2, child: Text(rank, style: const TextStyle(fontSize: 16)))]), title: Text(member.user?.name?? 'مستخدم'), subtitle: Text('نقاط: ${member.points} | ${member.role == 'owner'? 'المشرف' : 'عضو'}'), trailing: isOwner && member.role!= 'owner'? PopupMenuButton(itemBuilder: (context) => [PopupMenuItem(child: const Text('حظر'), onTap: () => _banMember(member.userId)), PopupMenuItem(child: const Text('إزالة'), onTap: () => _kickMember(member.userId))]) : null);
      });
    }))])));
  }
  ...
  Widget build(BuildContext context) {
    final isOfficial = widget.room.roomType == 'official';
    return Scaffold(extendBodyBehindAppBar: true, appBar: AppBar(backgroundColor: Colors.transparent, title: Text(widget.room.roomName), actions: [if (!isOfficial && isOwner) PopupMenuButton(icon: const Icon(Icons.more_vert), itemBuilder: (context) => [PopupMenuItem(child: const Text('عرض الأعضاء'), onTap: () => _showMembers()), PopupMenuItem(child: const Text('نسخ رابط الدعوة'), onTap: () => _copyInviteLink()), PopupMenuItem(child: const Text('حذف الغرفة', style: TextStyle(color: Colors.red)), onTap: () => _deleteRoom())]), if (!isJoined) TextButton(onPressed: _joinRoom, child: const Text('دخول', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold))), if (isJoined && (widget.room.roomType!= 'official' ||!isAdmin)) TextButton(onPressed: _leaveRoom, child: const Text('خروج', style: TextStyle(color: Colors.white, fontWeight: FontWeight.bold)))]), body: AppBackground(child: Column(children: [Expanded(child: StreamBuilder<List<MessageModel>>(stream: repo.getRoomMessagesStream(roomId: widget.room.id), builder: (context, snapshot) {
      if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
...
class ProfileScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final repo = SupabaseRepository();
    return Scaffold(backgroundColor: Colors.transparent, appBar: AppBar(backgroundColor: Colors.transparent, title: const Text('الملف الشخصي')), body: FutureBuilder<UserModel?>(future: repo.getCurrentUser(), builder: (context, snapshot) {
...
class _AppDrawerState extends State<AppDrawer> with SingleTickerProviderStateMixin {
  ...
  @override
  Widget build(BuildContext context) {
    return Drawer(backgroundColor: Colors.transparent, child: AppBackground(child: ListView(padding: EdgeInsets.zero, children: [DrawerHeader(child: GlassCard(child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [GestureDetector(onTap: () {
      if (_animationController.isCompleted) {_animationController.reverse();} else {_animationController.forward();}
      Scaffold.of(context).closeDrawer();
    }, child: AnimatedBuilder(animation: _animation, builder: (context, child) => Column(mainAxisSize: MainAxisSize.min, children: [Transform.rotate(angle: _animation.value * 0.785, child: Container(width: 30, height: 3, decoration: BoxDecoration(color: AppColors.button, borderRadius: BorderRadius.circular(2)))), SizedBox(height: _animation.value * 8 + 6), Transform.rotate(angle: -_animation.value * 0.785, child: Container(width: 30, height: 3, decoration: BoxDecoration(color: AppColors.button, borderRadius: BorderRadius.circular(2))))]))), const SizedBox(height: 12), const Text(AppConfig.appName, style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: AppColors.button))]))), if (isAdmin) ListTile(leading: const Icon(Icons.admin_panel_settings_rounded, color: AppColors.button), title: const Text('لوحة التحكم', style: TextStyle(fontWeight: FontWeight.bold)), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AdminPanel()))), ListTile(leading: const Icon(Icons.add_home_work_outlined, color: AppColors.icon), title: const Text('طلب إنشاء غرفة'), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const CreateRoomRequestScreen()))), ListTile(leading: const Icon(Icons.privacy_tip_outlined, color: AppColors.icon), title: const Text('سياسة الخصوصية'), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const PrivacyPolicyScreen()))), ListTile(leading: const Icon(Icons.mail_outline_rounded, color: AppColors.icon), title: const Text('تواصل معنا'), onTap: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const ContactUsScreen()))), const Divider(), Padding(padding: const EdgeInsets.all(16), child: Text('© ${AppConfig.copyrightYear} ${AppConfig.copyrightName}. All rights reserved.', style: const TextStyle(color: AppColors.textLight, fontSize: 12), textAlign: TextAlign.center))])));
  }
}
...
class _AdminPanelState extends State<AdminPanel> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(title: const Text('لوحة التحكم')), body: AppBackground(child: StreamBuilder<List<Map<String, dynamic>>>(stream: supabase.from('profiles').stream(primaryKey: ['id']).order('created_at'), builder: (context, snapshot) {
      if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
      final users = snapshot.data!;
      return ListView.builder(padding: const EdgeInsets.all(16), itemCount: users.length, itemBuilder: (context, i) {
        final user = users[i];
        final isBanned = user['is_banned'] == true;
        return GlassCard(margin: const EdgeInsets.only(bottom: 12), child: ListTile(leading: CircleAvatar(backgroundImage: user['avatar_url']!= null? NetworkImage(user['avatar_url']) : null, child: user['avatar_url'] == null? Text(user['name']?[0]?? 'U') : null), title: Text(user['name']?? 'مستخدم'), subtitle: Text(user['email']?? ''), trailing: isBanned? TextButton(onPressed: () => _unbanUser(user['id']), child: const Text('رفع الحظر', style: TextStyle(color: Colors.green))) : TextButton(onPressed: () => _banUser(user['id']), child: const Text('حظر', style: TextStyle(color: Colors.red)))));
      });
    })));
  }
}
...
class CreateRoomRequestScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('طلب إنشاء غرفة')), body: AppBackground(child: Center(child: GlassCard(margin: const EdgeInsets.all(24), child: const Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.construction, size: 60, color: AppColors.button), SizedBox(height: 16), Text('قريباً', style: TextStyle(fontSize: 24, fontWeight: FontWeight.bold)), SizedBox(height: 8), Text('هذه الميزة قيد التطوير', style: TextStyle(color: AppColors.textLight))])))));
  }
}
...
class PrivacyPolicyScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('سياسة الخصوصية')), body: AppBackground(child: ListView(padding: const EdgeInsets.all(16), children: const [GlassCard(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('سياسة الخصوصية', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold)), SizedBox(height: 16), Text('نحن نحترم خصوصيتك. لا نشارك بياناتك مع أي طرف ثالث.')]))])));
  }
}
...
class ContactUsScreen extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Scaffold(appBar: AppBar(title: const Text('تواصل معنا')), body: AppBackground(child: Center(child: GlassCard(margin: const EdgeInsets.all(24), child: const Column(mainAxisSize: MainAxisSize.min, children: [Icon(Icons.email, size: 60, color: AppColors.button), SizedBox(height: 16), Text('support@seachat.app', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold))])))));
  }
}
